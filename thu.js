    ...
    webviewView.webview.onDidReceiveMessage(async (data) => {
      ...
      switch (text) {
        ...
        case "showNotification": {
          ...
          if (data.error) {
            this.logError(notificationText, undefined, true);
          } else {
            this.logInfo(notificationText, true);
          }
...
private setupUserDataChangeListener() {
  const userDataChangeDisposable = this.extensionContext.globalState.onDidChange((event) => {
    if (event.key === "userData") {
      const currentUserData = this.extensionContext.globalState.get<string>("userData");
      SidebarWebViewProvider.broadcastToAllWebviews({
        command: "userDataResponse",
        user: currentUserData ? JSON.parse(currentUserData) : null,
      });
      this._outputChannel.appendLine("🔄 Detected user data change and broadcasted to all webviews");
    }
  });
  this.extensionContext.subscriptions.push(userDataChangeDisposable);
}
constructor(extensionUri: Uri, public extensionContext: ExtensionContext) {
    // ... existing constructor code
    this.setupUserDataChangeListener();
}
